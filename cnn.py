...
with graph.as_default():
    ...
    def model(data, weight, bias, dropout=1):
        ...
        fc1 = tf.nn.relu(tf.matmul(reshape, weight['w3'])+bias['b3'])
        fc2 = tf.nn.relu(tf.matmul(fc1, weight['w4'])+bias['b4'])
        out = tf.matmul(fc2, weight['w5'])+bias['b5']
        return out
    ...
    weight = {
        ...
        'w3': tf.Variable(tf.random_normal([50*5*5, 500])),
        'w4': tf.Variable(tf.random_normal([500, 84])),
        'w5': tf.Variable(tf.random_normal([84, num_labels]))
    }
    ...
    bias = {
        ...
        'b5': tf.Variable(tf.random_normal([num_labels]))
    }

    logits = model(tf_data, weight, bias, keep_prob)
    ...
    # accuracy
    correct_pre = tf.equal(tf.argmax(logits, axis=1), tf.argmax(tf_labels, axis=1))
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps):
        ...
        feed_dict={tf_data: batch_data, tf_labels: batch_label, keep_prob: 0.5}
        _, l, train_accuracy, summary = session.run([optimizer, loss, accuracy, merged], feed_dict=feed_dict)
        writer.add_summary(summary, step)
        if step % 1000 == 0:
            valid_accuracy = 0
            for i in range(math.ceil(valid_data.shape[0] / batch_size)):
                valid_offset = i * batch_size % (valid_data.shape[0] - batch_size)
            ...
            print('Valid accuracy: %f%%' % (100 * valid_accuracy / math.floor(valid_data.shape[0] / batch_size)))
...
with tf.Session(graph=graph) as session:
    ...
    print('Test accuray: %f%%' % (100 * test_accuracy / math.floor(test_data.shape[0] / batch_size)))